...
function updateLineNumber(e) {
    ...
    linesCount = codeInput.childElementCount || 1
 ...
 * 
 * @param {string} text - The text to be inserted. If null, empty string will be inserted.
 * @returns {void}
 */
function insertNewLineCode(text) {
    ...
    window.getSelection().setPosition(newLine, 0)
...
/**
 * Create a new div element for the first line in codeInput element
 * 
 * Problem: 
 * - By default in the content editable, the first line is a textnode
 * - After press enter, browser will add new a div for new line automaticaly
 * - This will make the line system inconsistent and raises another bug
 * 
 * This function will create a new div for the first line to wrap the textnode
 * 
 * @param {*} e - The keydown event
 */
function createFirstLine(e) {
    e.target.innerHTML = '' // Remove <br/> that causes the firstline not inserted corectly
...
/**
 * Remove <br> in the new line after Enter
 * 
 * The problem arises when user presses enter, 
 * new line is generated by the browser as a div element with a <br> tag inside. 
 * This <br> tag can cause several issues in the code editor. 
 * This code editor doesn't use Line Break for Multi Lines System, it uses One DIV One line System instead.
 * 
 */
function removeBreakLine() {
    const breakLines = document.querySelectorAll('br')
    breakLines.forEach(br => {
        br.parentElement.innerHTML = ' '
        // br.remove()
    })
}
...
codeInput.addEventListener('keydown', (e) => {
    ...
    if (codeInput.childElementCount === 0 || codeInput.firstChild.tagName !== 'DIV') {
        createFirstLine(e)
    }
    ...
    if (!e.shiftKey && !e.ctrlKey && e.key == 'Enter') {
        ...
        setHeightCodeInput(linesCount)
        codeInput.addEventListener('keyup', removeBreakLine, {once:true})
    }
    ...
    if (e.key == 'Backspace') {
        ...
        if (window.getSelection().type === 'Range') {
            codeInput.addEventListener('keyup', updateLineNumber, {once:true})
        }
    }
